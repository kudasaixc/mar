...
async function choose(terminal, question, labels) {
    stdout.write(`\n${question}\n`);
...
async function configureProvider(terminal, providers) {
    ...
    if (preset.id === "custom") {
        ...
        if (env)
            config.apiKeyEnv = env;
    }
    providers[id] = config;
    return { id, modelHint: preset.modelHint };
}
async function configureAgent(terminal, providers, role, index = 0) {
    let providerId;
...
export async function runOnboarding(terminal) {
    ...
    const approval = ["on-request", "never", "read-only"][approvalChoice];
    const config = {
        version: 1,
        providers,
        team: { master, workers },
