...
import type { ApprovalHandler, ToolDefinition } from "../tools/types.js";
...
export class Orchestrator {
  ...
  private readonly notes: SharedNotes;

  private constructor(workspace: Workspace, master: AgentRuntime, workers: Map<string, AgentRuntime>, notes: SharedNotes) {
  ...
  static async create(options: OrchestratorOptions): Promise<Orchestrator> {
    ...
    const onEvent: RuntimeEventHandler = options.onEvent ?? (() => undefined);
    ...
    const workers = new Map<string, AgentRuntime>();
    for (const profile of config.team.workers) {
      ...
      for (const tool of pluginTools) registry.add(tool);
    ...
    for (const tool of pluginTools) masterRegistry.add(tool);
    if (workers.size) masterRegistry.add(delegateTool(workers));
  ...
  reset(): void {
    this.master.reset();
    for (const worker of this.workers.values()) worker.reset();
  }
...
function delegateTool(workers: ReadonlyMap<string, AgentRuntime>): ToolDefinition {
  return {
    spec: {
      name: "delegate",
    ...
    execute: async (args) => {
      ...
      if (!worker) return { ok: false, output: `Unknown worker "${name}". Available: ${[...workers.keys()].join(", ")}` };
      if (!task) return { ok: false, output: "Delegated task cannot be empty." };
      try {
