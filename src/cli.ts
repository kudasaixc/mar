...
function parse(argv: string[]): { command: string; positional: string[]; options: CliOptions } {
  const positional: string[] = [];
  const options: CliOptions = { dir: process.cwd(), yes: false, readOnly: false, available: false };
  for (let index = 0; index < argv.length; index += 1) {
    const arg = argv[index]!;
    if (arg === "--dir" || arg === "-d") options.dir = resolve(argv[++index] ?? "");
    else if (arg === "--config") options.config = resolve(argv[++index] ?? "");
    else if (arg === "--yes" || arg === "-y") options.yes = true;
    else if (arg === "--read-only") options.readOnly = true;
    else if (arg === "--available") options.available = true;
    else positional.push(arg);
  }
  const known = new Set(["init", "run", "chat", "models", "doctor", "config", "completion", "help"]);
  const first = positional[0];
  if (!first) return { command: stdin.isTTY ? "chat" : "help", positional: [], options };
  if (first === "--help" || first === "-h") return { command: "help", positional: [], options };
  if (first === "--version" || first === "-v") return { command: "version", positional: [], options };
  if (known.has(first)) return { command: first, positional: positional.slice(1), options };
...
async function ensureConfiguration(): Promise<void> {
  if (await hasConfig()) return;
  if (!stdin.isTTY) throw new ConfigurationError(`No config at ${GLOBAL_CONFIG_PATH}; run "mar init" interactively.`);
  const terminal = createTerminal();
  try { await runOnboarding(terminal); } finally { terminal.close(); }
}

async function createRuntime(options: CliOptions, terminal = createTerminal()) {
...
async function runOnce(task: string, options: CliOptions): Promise<void> {
  if (!task.trim()) throw new Error("A task is required. Example: mar run \"add unit tests\"");
  const terminal = createTerminal();
  try {
...
async function chat(options: CliOptions): Promise<void> {
  ...
  try {
    ...
    for (;;) {
      ...
      if (input === "/agents") { stdout.write(`${runtime.teamStatus()}\n`); continue; }
      if (input === "/diff") { stdout.write(`${await runtime.workspace.diff()}\n`); continue; }
      if (input === "/notes") { stdout.write(`${runtime.sharedNotes()}\n`); continue; }
      if (input === "/clear") { runtime.reset(); stdout.write("Conversation context cleared.\n"); continue; }
...
async function showModels(options: CliOptions): Promise<void> {
  await ensureConfiguration();
...
async function doctor(options: CliOptions): Promise<void> {
  ...
  try { await access(options.dir, constants.R_OK | constants.W_OK); checks.push(["Workspace", true, resolve(options.dir)]); }
  catch { checks.push(["Workspace", false, `not readable/writable: ${options.dir}`]); }
  try {
    const { stdout: gitVersion } = await execFileAsync("git", ["--version"]);
    checks.push(["Git", true, gitVersion.trim()]);
  } catch { checks.push(["Git", false, "not found (optional, but recommended)"]); }
  try {
    const { config, paths } = await loadConfig(options.dir, options.config);
    checks.push(["Configuration", true, paths.join(" + ")]);
    for (const [id, provider] of Object.entries(config.providers)) {
      if (provider.apiKeyEnv) checks.push([`Key ${id}`, Boolean(process.env[provider.apiKeyEnv]), provider.apiKeyEnv]);
      else checks.push([`Provider ${id}`, true, provider.baseUrl ?? provider.kind]);
    }
  } catch (error) {
    checks.push(["Configuration", false, error instanceof Error ? error.message : String(error)]);
  }
  for (const [name, ok, detail] of checks) stdout.write(`${ok ? colors.green("✓") : colors.red("✗")} ${name}: ${detail}\n`);
  if (checks.some(([, ok]) => !ok)) process.exitCode = 1;
}

function completion(shell: string): string {
  const commands = "init run chat models doctor config completion";
  if (shell === "bash") return `complete -W "${commands}" mar`;
  if (shell === "zsh") return `#compdef mar\n_arguments '1:command:(${commands.replaceAll(" ", " ")})'`;
  if (shell === "fish") return commands.split(" ").map((command) => `complete -c mar -f -a ${command}`).join("\n");
...
async function main(): Promise<void> {
  ...
  switch (command) {
    ...
    case "run": await runOnce(positional.join(" "), options); break;
    case "chat": await chat(options); break;
    case "models": await showModels(options); break;
    case "doctor": await doctor(options); break;
    case "config": {
      ...
      } else stdout.write(`${GLOBAL_CONFIG_PATH}\n`);
      break;
    }
    case "completion": stdout.write(`${completion(positional[0] ?? "")}\n`); break;
